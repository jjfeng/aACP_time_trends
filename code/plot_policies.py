...
def main(args=sys.argv[1:]):
    ...
    ordered_approval_history_keys = (
            sorted([k for k in approval_history_keys]) if
    ("Fixed" not in approval_history_keys) else (sorted([k for k in
        approval_history_keys if k != "Fixed"]) + ["Fixed"]))
    human_max_losses = []
    ...
    plot_losses(
        ...
        ymin=args.y_min,
        ymax=max(human_max_loss + 0.1, args.y_max),
