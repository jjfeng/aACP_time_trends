...
def run_shift_check(model, dr_model, path, fields, fields_dr, criterion, target_func=None):
    model.eval()
    dr_model.eval()
    ...
    test_iterator = data.Iterator(
        test_data_ratings,
        batch_size = len(test_data_ratings),
        shuffle=False)
    test_iterator_dr = data.Iterator(
        test_data_dr,
        batch_size = len(test_data_dr),
        shuffle=False)
    ...
    for batch in test_iterator_dr:
        ...
        print("EFFECTIVE SAMPLE SIZE", np.power(np.sum(density_ratios),2)/np.sum(np.power(density_ratios, 2)))
    ...
    adjusted_loss = np.mean(test_losses.flatten() * density_ratios.flatten())/np.mean(density_ratios)
    print("  ADJUSTED LOSS", adjusted_loss)
    return adjusted_loss

def run_shift_test(model, dr_model, path, fields, fields_dr, criterion, target_func=None):
    model.eval()
    dr_model.eval()
    test_data_ratings = data.TabularDataset(path = path, format = 'json', fields = fields)
    test_data_dr = data.TabularDataset(path = path, format = 'json', fields = fields_dr)
...
def main(args=sys.argv[1:]):
    ...
    orig_year = 2008
    new_year = 2009
    YELP_DENSITY_TRAIN = "data/yelp_academic_dataset_review_year_train_2008_%d.json" % new_year
    ...
    fields_dr = {"text": ('text', TEXT1), "year": ('label', YEAR_LABEL1), "stars": ('star_label',LABEL),}

    torch.manual_seed(0)
    dr_xy_model = estimate_xy_density_ratio(YELP_DENSITY_TRAIN, fields_dr, N_EPOCHS=N_EPOCHS_DR, num_hidden=num_hidden_dr, embed_dim=embed_dim, split_ratio=split_ratio_dr, actual_embed_dim=actual_embed_dim)
    dr_model = estimate_density_ratio(YELP_DENSITY_TRAIN, fields_dr, N_EPOCHS=N_EPOCHS_DR, num_hidden=num_hidden_dr, embed_dim=embed_dim, split_ratio=split_ratio_dr, actual_embed_dim=actual_embed_dim)

    rating_criterion = nn.L1Loss()
    torch.manual_seed(0)
    rating_model = train_rating_model(YELP_RATING_TRAIN, fields, rating_criterion, N_EPOCHS=N_EPOCHS, split_ratio=0.9, num_hidden=num_hidden, embed_dim=embed_dim, actual_embed_dim=actual_embed_dim)

    criterion = nn.L1Loss(reduce=False)
    print("NO SHIFT", orig_year)
    test_losses = run_test(rating_model, YELP_TEST, fields, criterion)
    print("  TEST LOSS %.3f %.3f" % (np.mean(test_losses), np.sqrt(np.var(test_losses)/test_losses.size)))
    print("TRUE", new_year)
