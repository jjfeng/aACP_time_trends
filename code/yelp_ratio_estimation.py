...
def estimate_xy_density_ratio(YELP_TRAIN, fields, N_EPOCHS = 20, num_hidden=30, split_ratio=0.7, embed_dim=50, actual_embed_dim=2):
    ####
    # Load data
    ####
    ...
    model = TextYearModel(vocab_size = len(TEXT.vocab), vocab=TEXT.vocab, embed_dim = actual_embed_dim, num_class=2, num_hidden=num_hidden, freeze=True)
    ...
    for epoch in range(N_EPOCHS):
        train_loss = train_xy(model, train_iterator, optimizer, criterion, target_func=target_func)
...
def estimate_density_ratio(YELP_TRAIN, fields, N_EPOCHS = 20, num_hidden=30, split_ratio=0.7, embed_dim=50, actual_embed_dim=2):
    ####
    # Load data
    ####
    #YELP = "/Users/jeanfeng/Downloads/10100_1035793_bundle_archive/yelp_academic_dataset_review.json"
    ...
    # Load model
    #model = DensityRatioModel(vocab_size = len(TEXT.vocab), vocab=TEXT.vocab, embed_dim = EMBED_DIM, num_class=2, num_hidden=num_hidden)
    model = TextSentiment(vocab_size = len(TEXT.vocab), vocab=TEXT.vocab, embed_dim = actual_embed_dim, num_class=2, num_hidden=num_hidden, freeze=True)
    ...
    train_iterator, valid_iterator = data.Iterator.splits(
        ...
        shuffle=True)
    for epoch in range(N_EPOCHS):
        train_loss = train(model, train_iterator, optimizer, criterion, target_func=target_func)
        print(f'\tTrain Loss: {train_loss:.3f}')
...
def main(args=sys.argv[1:]):
    ...
    model = estimate_density_ratio(YELP_TRAIN, fields, N_EPOCHS = 45, num_hidden=10, embed_dim=50)
