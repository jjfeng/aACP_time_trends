...
def train_year(model, iterator, optimizer, criterion, target_func=None, year_func=None):
    ...
    for batch in iterator:
        ...
        year_label = (
            batch.year_label if year_func is None else year_func(batch.year_label)
        )
...
def train_year_rating_model(
    ...
    for epoch in range(N_EPOCHS):
        train_loss = train_year(
            model, train_iterator, optimizer, criterion, year_func=year_func
        )
...
def run_hypothesis_test(
    ...
    test_iterator0 = data.Iterator(
        test_data0, batch_size=len(test_data0), shuffle=False
    )
    test_iterator_year = data.Iterator(
        test_data_year, batch_size=len(test_data_year), shuffle=False
    )
...
def main(args=sys.argv[1:]):
    ...
    YELP_TRAIN_YEARS = (
        "data/yelp_academic_dataset_review_year_train_%d_%d.json" % tuple(years)
    )
    ...
    torch.manual_seed(0)
    model_year = train_year_rating_model(
        YELP_TRAIN_YEARS,
        fields_year,
        criterion,
        year_func,
    ...
    torch.manual_seed(0)
    model_blind = train_rating_model(
        YELP_TRAIN_YEARS,
        fields_blind,
        criterion,
        N_EPOCHS=n_epochs,
